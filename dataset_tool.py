...
def create_from_adobe_images(tfrecord_dir, image_dir, shuffle):
	print('Loading images from "%s"' % image_dir)
	image_filenames = sorted(glob.glob(os.path.join(image_dir, '*')))
	#compute num of classes
	label_set = set()
	for filename in image_filenames:
		_, name = os.path.split(filename)
		label_tag = name.split('_')[1]
		label_set.add(label_tag)
	label_list = list(label_set)

	print('num of category = {}'.format(len(label_list)))

	onehot = np.zeros((len(image_filenames), len(label_list)), dtype=np.float32)
	for id, filename in enumerate(image_filenames):
		_, name = os.path.split(filename)
		material_tag = name.split('_')[1]
		label = label_list.index(material_tag)
		onehot[id, label] = 1.0
	...
	with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
		...
		for idx in range(order.size):
			...
			#label
			img = np.concatenate([
				img_raw[:, 0:resolution], 
				img_raw[:, 2*resolution:3*resolution,0:1],
